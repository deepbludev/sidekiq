...
files_modified:
  ...
  - sidekiq-webapp/drizzle/XXXX_add_deleted_sidekiq_name.sql
autonomous: true
gap_closure: true
...
2. `pnpm db:push` applies migration successfully
3. Inspect database: `\d thread` shows `deleted_sidekiq_name` column
  </verify>
...
2. Remove the redundant NOT_FOUND check after the delete since we now check earlier.
