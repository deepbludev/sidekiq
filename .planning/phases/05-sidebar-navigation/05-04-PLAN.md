...
4. **Use highlight in thread title:**
   - Pass highlightMatch to ThreadItem OR render title directly in list
   - For simplicity, render title with highlight in the list:
```tsx
{item.type === "thread" && (
  <ThreadItem
    thread={item.thread}
    ...
    // Highlight can be applied via a prop or by wrapping title
    highlightTitle={isSearchActive ? highlightMatch : undefined}
  />
)}
```

5. **Update ThreadItem to optionally highlight title:**
   - Add optional `highlightTitle` prop to ThreadItem
   - If provided, use it to render title instead of plain text

**Alternative simpler approach:**
   - Don't modify ThreadItem
   - Create a wrapper in SidebarThreadList that renders the highlighted title alongside ThreadItem
   - This keeps ThreadItem unchanged
