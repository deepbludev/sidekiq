...
5. **Wire onThreadSelect using event delegation pattern:**

Use a click handler on the virtualized item wrapper that calls onThreadSelect whenever a thread is clicked. ThreadItem already handles its own navigation via router.push(), so we just need to also trigger the mobile close callback:

```tsx
// In the virtualized items render:
{virtualizer.getVirtualItems().map((virtualRow) => {
  ...
  return (
    <div
      ...
      // Event delegation: call onThreadSelect when clicking thread items
      onClick={item.type === "thread" ? () => onThreadSelect?.() : undefined}
    >
      {item.type === "header" ? (
...
This pattern:
- Keeps ThreadItem unchanged (no prop additions needed)
- ThreadItem handles navigation internally via router.push()
- Parent wrapper calls onThreadSelect() to close mobile drawer
- Event delegation is efficient (one handler per item, not per child)
...
- All Phase 5 SIDE requirements met:
  ...
  - SIDE-06: DEFERRED to Phase 7 (threads don't have sidekiqId until Sidekiq Chat Integration)
</success_criteria>
