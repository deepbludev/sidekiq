...
In sidekiq-webapp/src/app/api/chat/route.ts, update thread activeModel when saving messages.

Find the section where the assistant message is saved (around line 187-200) and add activeModel update:

```typescript
// Update thread lastActivityAt, increment message count, and set activeModel
await db
  .update(threads)
  .set({
...
Also update the new thread creation (around line 116-122) to include activeModel:
```typescript
...
- Chat route updates activeModel: `grep -n "activeModel" sidekiq-webapp/src/app/api/chat/route.ts`
- TypeScript compiles: `cd sidekiq-webapp && pnpm typecheck`
  </verify>
...
Add a modelPicker prop to render the picker in the input area:
...
1. Add model switch tracking state:
```typescript
...
2. Use the model selection hook:
```typescript
const {
  selectedModel,
  setSelectedModel,
...
3. Update transport to include model in body:
```typescript
...
4. Update handleSubmit to pass model:
```typescript
const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
  ...
  await sendMessage({ text: trimmedInput }, { body: { model: selectedModel } });
};
```

5. Add model picker to ChatInput:
```typescript
<ChatInput
  input={input}
  setInput={setInput}
...
6. Update MessageList rendering to include model switch hints:
In the message area, render hints between messages where switches occurred:
```typescript
<MessageList
  messages={messages}
  streamingMessageId={...}
  ...
  modelSwitchHints={modelSwitches}
/>
```

Alternatively, render hints inline by mapping over messages with interleaved hints.
  </action>
  <verify>
...
- Model passed to sendMessage: `grep -n "model.*selectedModel" sidekiq-webapp/src/components/chat/chat-interface.tsx`
- TypeScript compiles: `cd sidekiq-webapp && pnpm typecheck`
...
- Selected model passed to /api/chat in request body
- Thread activeModel updated on each message
