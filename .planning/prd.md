...
- 1.0.2. tRPC setup with type-safe routes and Zod validation
- 1.0.3. Database setup (PostgreSQL) with Drizzle ORM
- 1.0.4. Drizzle migrations for core data models (Users, Teams, TeamMembers, TeamInvites, Sidekiqs, Threads, Messages)
- 1.0.6. Vercel Blob Storage setup for avatar uploads

#### Phase 1.1. Authentication
- 1.1.1. Better-Auth integration
- 1.1.2. Email/Password authentication with validation
- 1.1.3. Password reset flow with email tokens (1-hour expiration)
- 1.1.4. Protected routes and middleware for authenticated pages

#### Phase 1.2. Core Chat
- 1.2.1. Basic chat UI with sidebar (history) and main chat area
- 1.2.2. Integration with LLM providers (OpenAI/Anthropic) using Vercel AI SDK
- 1.2.3. Server-Sent Events (SSE) for streaming AI responses
- 1.2.4. Message persistence with enhanced schema (model, tokens, metadata, parentMessageId)
- 1.2.5. Thread management: create, auto-title generation after first exchange
- 1.2.6. Model picker (dropdown near input) with persistent selection per thread
- 1.2.7. Delete a thread (permanent with confirmation dialog)
- 1.2.8. Archive a thread (soft delete, recoverable)
- 1.2.9. Pin a thread (sticky to top of sidebar)
- 1.2.10. Edit a message (always creates branch with parent-child references)
- 1.2.11. Message regeneration (regenerate last AI response)
- 1.2.12. Conversation branching navigation (< 2 / 5 > controls)
- 1.2.13. Optimistic UI for message sending with error rollback
- 1.2.14. Empty states with CTAs for new users
- 1.2.15. Loading states (skeletons, streaming indicators)
- 1.2.16. Scroll position preservation when switching threads
- 1.2.17. Sidebar search functionality (search threads by title/content)
- 1.2.18. Date grouping in sidebar (Today/Yesterday/This Week/Older)

#### Phase 1.3. Sidekiqs (The "Gem" Feature)
- 1.3.1. CRUD pages for Sidekiqs with validation (max 4000 char instructions, 100 char names)
- 1.3.2. Text initial avatars for Sidekiqs (colored circles with initials)
- 1.3.3. Chat with a Sidekiq (instructions prepended as system message)
- 1.3.4. Sidekiq visual indicators in sidebar (icon, badge, subtitle with Sidekiq name)
- 1.3.5. Rate limiting: max 100 Sidekiqs per user, 10 creations per hour
- 1.3.6. Empty states for users with no Sidekiqs
- 1.3.7. Organize Sidekiqs in folders (optional, may defer)
...
#### Phase 1.5. Teams
- 1.5.1. Team creation and management pages
- 1.5.2. Token-based team member invites (email with secure link, 7-day expiration)
- 1.5.3. Team member management (view members, roles, remove members)
- 1.5.4. Sidekiq team sharing logic (canTeamEdit flag for permissions)
- 1.5.5. Team Sidekiq permissions: view/use for all, edit based on canTeamEdit flag
- 1.5.6. Team deletion flow (transfers team Sidekiqs to owner with confirmation)
- 1.5.7. Team authorization middleware for secure access control

### Version 2. Premium

#### Phase 2.1. Advanced Chat Features: Tools
- 2.1.1. Web Search: real-time information retrieval.
- 2.1.2. Image Generation: generating images on demand.
- 2.1.3. Deep Research: complex multi-step reasoning/research tasks.

#### Phase 2.2. Advanced Chat Features: Branching
- 2.2.1. Users can edit any user message to fork the conversation tree.
- 2.2.2. Users can navigate between branches (e.g., `< 2 / 5 >` navigation controls).

#### Phase 2.3. Advanced Chat Features: Other Features
- 2.3.1. Share a chat with link (copy link to chat).

#### Phase 2.4. Projects: Organize chats
- 2.4.1. Projects management: create, edit, delete projects.
- 2.4.2. Project membership: add/remove members to a project.
- 2.4.3. Project visibility: public/private.
- 2.4.4. Project settings: edit project name, description, avatar, etc.
- 2.4.5. Project sharing: share a project with team members.
- 2.4.6. Project view/edit/create permissions within a project.

#### Phase 2.5: Payments & Billing
- 2.5.1. Database migrations for CreditBalances, CreditTransactions, UsageRecords, Subscriptions tables
- 2.5.2. Stripe integration (Checkout Sessions, Webhooks) for top-ups
- 2.5.3. Base subscription plan ($20/month with $20 credits, no rollover)
- 2.5.4. Credit deduction middleware (asynchronous, after response delivery)
- 2.5.5. Usage tracking per message (tokens, model, cost calculation from ENV variables)
- 2.5.6. User & team credit balance management (dual-level tracking)
- 2.5.7. Insufficient credits handling (auto-top-up if configured, else block with prompt)
- 2.5.8. Negative balance grace period (-$5 max)
- 2.3.9. Auto-top-up configuration and background jobs (Inngest or Vercel Cron)
- 2.3.10. Usage dashboard: view usage by model with graphs (total usage + usage over time)
- 2.3.11. Stripe webhook handlers with signature validation and idempotency

#### Phase 2.6. General Improvements
- 2.6.1. Social login (Google).
- 2.6.2. Vercel Observability for monitoring and logging.
- 2.6.3. UI/UX improvements and refinements.
---
